...
public class HtmlToWikiConverter {
    ...
    public List<String>  htmlStreamToWiki(Reader reader, String path) throws Exception {
        ...
        htmlParser.pop();
        reader.close();
        wikiValue = htmlParser.wikiFinal;
        return wikiValue;
    }
...
class HTMLParser extends HTMLEditorKit.ParserCallback {
    ...
    private boolean isIgnoredText = false;
    ...
    List<StringBuilder> wikiStack = new ArrayList<>();
    StringBuilder wikiText = new StringBuilder();
    
    public HTMLParser(String fullUrl) {
        ...
        if (t == HTML.Tag.A) {
            ...
            if (lText.length()>0) {
                ...
                if (href!=null && href.length()>0)  {
                    wikiText.append("|");
                    wikiText.append(convertToGlobalUrl(href));
                }
                wikiText.append("]");
            }
            isIgnoredText = false;
    ...
    public void push() {
        wikiStack.add(wikiText);
        wikiText = new StringBuilder();
    }
    
    public void pop() {
        String result = wikiText.toString().trim();
        if (result.length()>0) {
        ...
        if (wikiStack.size()>0) {
            wikiText = wikiStack.remove(wikiStack.size()-1);
        }
    }
    ...
    public String convertToGlobalUrl(String relativeUrl) {
        if (relativeUrl.startsWith("http")) {
            ...
            System.out.println("++ URL from page was already global: "+relativeUrl);
            return relativeUrl;
        }
        
        // site global or root relative URL starts with a slash
        if (relativeUrl.startsWith("/")) {
            System.out.println(String.format("++ URL root (%s) / (%s)",siteRootPath(), relativeUrl));
        ...
        System.out.println(String.format("++ URL became (%s) / (%s)",containerPath, relativeUrl));
